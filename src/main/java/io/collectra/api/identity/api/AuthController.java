...
public class AuthController {
    ...
    ResponseEntity<AuthService.AuthTokens> register(@Valid @RequestBody RegisterRequest r) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(auth.register(r.slug(), r.companyName(), r.email(), r.password()));
    }

    @PostMapping("/login")
    AuthService.AuthTokens login(@Valid @RequestBody LoginRequest r) {
        return auth.login(r.tenantId(), r.email(), r.password());
    }

    @PostMapping("/refresh")
    AuthService.AuthTokens refresh(@Valid @RequestBody TokenRequest r) {
    ...
    public record RegisterRequest(
            @Pattern(regexp = "[a-z0-9-]{3,80}") String slug,
            @NotBlank @Size(max = 200) String companyName,
            @Email @NotBlank String email,
            @Size(min = 12, max = 72) String password) {}

    public record LoginRequest(
            @NotNull UUID tenantId, @Email @NotBlank String email, @NotBlank String password) {}
