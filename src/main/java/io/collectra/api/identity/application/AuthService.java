...
public class AuthService {
    ...
    public AuthTokens login(String tenantSlug, String email, String password) {
        String normalizedSlug =
                tenantSlug == null ? null : tenantSlug.trim().toLowerCase(Locale.ROOT);
        if (normalizedSlug == null || normalizedSlug.isBlank()) {
            throw new BadCredentialsException("Invalid credentials");
        }
        UUID tenantId =
                tenants.findBySlugIgnoreCase(normalizedSlug)
                        .map(Tenant::getId)
    ...
    private AuthTokens loginResolved(UUID tenantId, String email, String password) {
        ...
        limiter.check(
                "login:" + tenantId + ":" + email.toLowerCase(Locale.ROOT),
                5,
    ...
    public AuthTokens refresh(String rawToken) {
        RefreshSession old =
                sessions.findByTokenHashForUpdate(hash(rawToken))
        ...
        UserAccount user =
                users.findById(old.getUserId())
                        .filter(u -> "ACTIVE".equals(u.getStatus()))
                        .orElseThrow(InvalidRefreshTokenException::new);
        TenantMembership membership =
                memberships
                        .findByIdAndTenantId(old.getMembershipId(), old.getTenantId())
                        .filter(TenantMembership::active)
                        .orElseThrow(InvalidRefreshTokenException::new);
    ...
    private AuthTokens issue(
        ...
        String access =
                jwt.issue(
                        user,
                        membership,
                        rbac.roleCodes(membership.getId()),
    ...
    private void requireActiveTenantForLogin(UUID tenantId) {
        boolean active = tenants.findById(tenantId).map(Tenant::active).orElse(false);
        if (!active) {
            throw new BadCredentialsException("Invalid credentials");
        }
    }
    ...
    private void requireActiveTenantForRefresh(UUID tenantId) {
        boolean active =
                tenantId != null && tenants.findById(tenantId).map(Tenant::active).orElse(false);
        if (!active) {
            throw new InvalidRefreshTokenException();
        }
    }
