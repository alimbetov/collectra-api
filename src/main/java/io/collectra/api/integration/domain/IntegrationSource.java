...
public class IntegrationSource extends AuditableEntity {
    ...
    private IntegrationSourceStatus status;

    @Column(name = "service_client_id", nullable = false)
    private UUID serviceClientId;

    @Column(name = "source_schema_definition_id", nullable = false)
    private UUID sourceSchemaDefinitionId;

    @Column(name = "mapping_profile_definition_id", nullable = false)
    private UUID mappingProfileDefinitionId;
    ...
    public void update(
            ...
            String resourcePolicy,
            String routingConfig) {
        if (status == IntegrationSourceStatus.ARCHIVED)
            throw new IllegalStateException("Archived source cannot be changed");
        this.name = name;
        this.serviceClientId = serviceClientId;
        this.sourceSchemaDefinitionId = sourceSchemaDefinitionId;
        this.mappingProfileDefinitionId = mappingProfileDefinitionId;
    ...
    public void activate() {
        if (status != IntegrationSourceStatus.DRAFT && status != IntegrationSourceStatus.SUSPENDED)
            throw new IllegalStateException("Only draft or suspended source can be activated");
        status = IntegrationSourceStatus.ACTIVE;
    }

    public void suspend() {
        if (status != IntegrationSourceStatus.ACTIVE)
            throw new IllegalStateException("Only active source can be suspended");
        status = IntegrationSourceStatus.SUSPENDED;
    }

    public void archive() {
        if (status == IntegrationSourceStatus.ARCHIVED)
            throw new IllegalStateException("Source is already archived");
        status = IntegrationSourceStatus.ARCHIVED;
    }
    ...
    public IntegrationSourceStatus getStatus() {
        return status;
    }
    ...
    public UUID getSourceSchemaDefinitionId() {
        return sourceSchemaDefinitionId;
    }

    public UUID getMappingProfileDefinitionId() {
        return mappingProfileDefinitionId;
    }
