...
public class IntegrationSourceController {
    ...
    List<IntegrationSourceService.SourceResponse> list() {
        return service.list(TenantContext.requireTenantId());
    }

    @GetMapping("/{id}")
    @PreAuthorize("hasAuthority('ROLE_HUMAN') and hasAuthority('INTEGRATION_SOURCE_READ')")
    IntegrationSourceService.SourceResponse get(@PathVariable UUID id) {
    ...
    IntegrationSourceService.SourceResponse create(@Valid @RequestBody CreateRequest r) {
        return service.create(
                ...
                new IntegrationSourceService.CreateCommand(
                        ...
                        r.name(),
                        r.serviceClientId(),
                        r.sourceSchemaDefinitionId(),
                        r.mappingProfileDefinitionId(),
                        r.processingMode(),
                        r.headerMapping(),
                        r.resourcePolicy(),
                        r.routingConfig()));
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAuthority('ROLE_HUMAN') and hasAuthority('INTEGRATION_SOURCE_MANAGE')")
    IntegrationSourceService.SourceResponse update(
            @PathVariable UUID id, @Valid @RequestBody UpdateRequest r) {
        return service.update(
                TenantContext.requireTenantId(),
                id,
                r.version(),
                new IntegrationSourceService.UpdateCommand(
                        r.name(),
                        r.serviceClientId(),
                        r.sourceSchemaDefinitionId(),
                        r.mappingProfileDefinitionId(),
    ...
    @PreAuthorize("hasAuthority('ROLE_HUMAN') and hasAuthority('INTEGRATION_SOURCE_READ')")
    IntegrationSourceService.ReadinessResponse readiness(@PathVariable UUID id) {
        return service.validate(TenantContext.requireTenantId(), id);
    }

    @PostMapping("/{id}/activate")
    @PreAuthorize("hasAuthority('ROLE_HUMAN') and hasAuthority('INTEGRATION_SOURCE_MANAGE')")
    IntegrationSourceService.SourceResponse activate(
            @PathVariable UUID id, @Valid @RequestBody VersionRequest r) {
        return service.activate(TenantContext.requireTenantId(), id, r.version());
    }

    @PostMapping("/{id}/suspend")
    @PreAuthorize("hasAuthority('ROLE_HUMAN') and hasAuthority('INTEGRATION_SOURCE_MANAGE')")
    IntegrationSourceService.SourceResponse suspend(
            @PathVariable UUID id, @Valid @RequestBody VersionRequest r) {
        return service.suspend(TenantContext.requireTenantId(), id, r.version());
    }

    @PostMapping("/{id}/archive")
    @PreAuthorize("hasAuthority('ROLE_HUMAN') and hasAuthority('INTEGRATION_SOURCE_MANAGE')")
    IntegrationSourceService.SourceResponse archive(
            @PathVariable UUID id, @Valid @RequestBody VersionRequest r) {
        return service.archive(TenantContext.requireTenantId(), id, r.version());
    }

    record CreateRequest(
            @NotBlank String code,
            @NotBlank String name,
            @NotNull UUID serviceClientId,
            @NotNull UUID sourceSchemaDefinitionId,
            @NotNull UUID mappingProfileDefinitionId,
