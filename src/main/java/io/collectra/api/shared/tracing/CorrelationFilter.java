...
public class CorrelationFilter extends OncePerRequestFilter {
    ...
    public CorrelationFilter(Tracer tracer) { this.tracer = tracer; }
    @Override protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        } finally { MDC.remove("correlationId"); MDC.remove("traceId"); }
    }
    private String valid(String value) {
        return value != null && value.matches("[A-Za-z0-9._-]{1,100}") ? value : UUID.randomUUID().toString();
    }
}
