...
@Entity
@Table(name = "customers", uniqueConstraints = @UniqueConstraint(name = "uk_customer_tenant_external", columnNames = {"tenant_id", "external_id"}))
public class Customer extends AuditableEntity {
    ...
    @Column(name = "first_name", length = 120) private String firstName;
    @Column(name = "last_name", length = 120) private String lastName;
    @Column(name = "middle_name", length = 120) private String middleName;
    ...
    @Column(name = "manager_user_id") private UUID managerUserId;
    @Column(name = "preferred_locale", length = 35) private String preferredLocale;
    @Column(length = 60) private String timezone;
    @JdbcTypeCode(SqlTypes.JSON) @Column(name = "custom_fields", columnDefinition = "jsonb") private JsonNode customFields;

    protected Customer() {}

    public Customer(UUID tenantId, String externalId, CustomerType customerType, String displayName,
                    String firstName, String lastName, String middleName, String companyName,
                    UUID managerUserId, String preferredLocale, String timezone, JsonNode customFields) {
    ...
    public void update(String displayName, String firstName, String lastName, String middleName,
                       String companyName, UUID managerUserId, String preferredLocale,
                       String timezone, JsonNode customFields) {
        this.displayName = required(displayName, "displayName");
        this.firstName = trim(firstName);
        this.lastName = trim(lastName);
        this.middleName = trim(middleName);
    ...
    public void changeStatus(CustomerStatus status) { this.status = java.util.Objects.requireNonNull(status); }
    ...
    public JsonNode getCustomFields() { return copy(customFields); }
    ...
    private static JsonNode copy(JsonNode v) { return v == null ? null : v.deepCopy(); }
}
