...
class TenantSecurityManagementIntegrationTest extends AbstractIntegrationTest {
    ...
    void permissionCatalogAndCustomRoleLifecycleAreEnforced() throws Exception {
        ...
        JsonNode updated =
                read(
                        put("/api/v1/identity/roles/{id}", role.get("id").asText())
                                .header("Authorization", bearer(admin.accessToken()))
        ...
        mockMvc.perform(
                        delete("/api/v1/identity/roles/{id}", role.get("id").asText())
                                .header("Authorization", bearer(admin.accessToken())))
                .andExpect(status().isConflict());

        JsonNode unused = createRole(admin.accessToken(), "UNUSED_" + randomCode(), "USER_READ");
        mockMvc.perform(
                        delete("/api/v1/identity/roles/{id}", unused.get("id").asText())
                                .header("Authorization", bearer(admin.accessToken())))
    ...
    void tenantAdminListsAndRevokesMembershipSessions() throws Exception {
        ...
        JsonNode sessions =
                read(
                        get("/api/v1/identity/memberships/{id}/sessions", membershipId)
                                .header("Authorization", bearer(admin.accessToken())));
        assertThat(sessions).hasSize(1);

        mockMvc.perform(
                        delete(
                                        "/api/v1/identity/memberships/{id}/sessions/{sessionId}",
                                        membershipId,
                                        sessions.get(0).get("id").asText())
                                .header("Authorization", bearer(admin.accessToken())))
                .andExpect(status().isNoContent());
        mockMvc.perform(
    ...
    private Auth register(String prefix) throws Exception {
        ...
        return new Auth(
                response.get("accessToken").asText(), response.get("refreshToken").asText(), email);
    }
