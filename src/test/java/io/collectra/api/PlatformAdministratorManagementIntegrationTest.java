...
class PlatformAdministratorManagementIntegrationTest extends AbstractIntegrationTest {
    ...
    void platformAdminCreatesListsAndBlocksAnotherAdministrator() throws Exception {
        ...
        JsonNode list =
                read(get("/api/v1/platform/administrators").header("Authorization", primaryBearer));
        assertThat(list.findValuesAsText("email")).contains(email);

        JsonNode secondTokens = login(email, "SecondPassword123!");
        mockMvc.perform(
                        patch(
                                        "/api/v1/platform/administrators/{id}/status",
    ...
    void administratorCannotBlockOrRemoveOwnRole() throws Exception {
        ...
        mockMvc.perform(
                        patch("/api/v1/platform/administrators/{id}/status", id)
                                .header("Authorization", bearer)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"active\":false,\"revision\":" + revision + "}"))
                .andExpect(status().isConflict());
        mockMvc.perform(
                        delete("/api/v1/platform/administrators/{id}/role", id)
                ...
                read(
                        post("/api/v1/platform/administrators")
                                ...
                                .content(
                                        "{\"email\":\""
                                                + email
                                                + "\",\"password\":\"OldPlatformPassword123!\"}"));
        JsonNode oldTokens = login(email, "OldPlatformPassword123!");
        mockMvc.perform(
                        put(
                                        "/api/v1/platform/administrators/{id}/password",
    ...
    private String subject(JsonNode tokens) {
        try {
            ...
            return json.readTree(java.util.Base64.getUrlDecoder().decode(parts[1]))
                    .get("sub")
                    .asText();
