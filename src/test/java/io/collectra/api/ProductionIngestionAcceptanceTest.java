...
class ProductionIngestionAcceptanceTest extends AbstractIntegrationTest {
    ...
    static void rabbit(DynamicPropertyRegistry r) {
        ...
        r.add("collectra.messaging.outbox-enabled", () -> "true");
    ...
    @Autowired IngestionBatchRepository batches;
    @Autowired IngestionRecordDiagnosticRepository diagnostics;
    ...
    void durableHappyPathAndTransportReplayReachCanonicalInvoice() throws Exception {
        ...
        var first =
                ingestion.reserve(
                        f.tenantId, f.clientId, f.sourceCode, "idem-1", "req-1", "text/csv", body);
        var replay =
                ingestion.reserve(
                        f.tenantId, f.clientId, f.sourceCode, "idem-1", "req-2", "text/csv", body);
        assertThat(replay.ingestionId()).isEqualTo(first.ingestionId());
        ...
        assertThat(
                        diagnostics
                                .findByTenantIdAndBatchIdAndRecordOrder(
                                        f.tenantId, first.ingestionId(), 1)
                                .orElseThrow()
                                .getOutcome())
                .describedAs(recordDiagnostic(f.tenantId, first.ingestionId(), 1))
                .isEqualTo("CREATED");
    }
    ...
    void businessReplayIsReusedAndChangedCanonicalStateIsConflict() throws Exception {
        ...
        var one =
                ingestion.reserve(
                        ...
                        "text/csv",
                        csv("INV-X", "100.00"));
        awaitTerminal(f.tenantId, one.ingestionId());
        var same =
                ingestion.reserve(
                        f.tenantId,
                        f.clientId,
        ...
        try {
            ...
            assertThat(
                            batches.findAll().stream()
                                    .filter(
                                            x ->
                                                    x.getTenantId().equals(f.tenantId)
                                                            && x.getIdempotencyKey()
                                                                    .equals("race-key")))
                    .hasSize(1);
        } finally {
    ...
    private void awaitTerminal(UUID tenantId, UUID id) throws Exception {
        ...
        while (System.nanoTime() < deadline) {
            var b = batches.findByIdAndTenantId(id, tenantId).orElseThrow();
            if (Set.of(
                            IngestionStatus.COMPLETED,
                            IngestionStatus.PARTIALLY_COMPLETED,
                            IngestionStatus.FAILED)
                    .contains(b.getStatus())) return;
            Thread.sleep(100);
        }
    ...
    private String recordDiagnostic(UUID tenantId, UUID batchId, int order) {
        return diagnostics
                .findByTenantIdAndBatchIdAndRecordOrder(tenantId, batchId, order)
                .map(value -> value.getErrorCode() + ": " + value.getSafeErrorMessage())
                .orElse("record diagnostic is missing");
    }

    private Fixture fixture(String prefix) {
        Tenant tenant =
                tenants.saveAndFlush(new Tenant("vc3-" + prefix + "-" + UUID.randomUUID(), "VC3"));
        ServiceClient client =
                clients.saveAndFlush(
                        new ServiceClient(
                                ...
                                null));
        SourceSchemaDefinition sd =
                schemaDefinitions.saveAndFlush(
                        new SourceSchemaDefinition(
                                tenant.getId(),
                                "S" + UUID.randomUUID().toString().replace("-", "").substring(0, 8),
                                "Schema"));
        SourceSchema schema =
                schemas.saveAndFlush(
                        new SourceSchema(
                                ...
                                SourceFormat.CSV,
                                1));
        SourceField invoice = field(schema, "Invoice", 1, true),
                customer = field(schema, "Customer", 2, false),
                number = field(schema, "Number", 3, false),
                due = field(schema, "DueDate", 4, false),
                amount = field(schema, "Amount", 5, false),
                currency = field(schema, "Currency", 6, false);
        schema.validated();
        schema.publish();
        schemas.saveAndFlush(schema);
        MappingProfileDefinition md =
                mappingDefinitions.saveAndFlush(
                        new MappingProfileDefinition(
                                tenant.getId(),
                                "M" + UUID.randomUUID().toString().replace("-", "").substring(0, 8),
                                "Mapping",
                                "INVOICE"));
        ...
        rule(mapping, invoice, "invoice.externalId", "TRIM");
        rule(mapping, customer, "customer.externalId", "TRIM");
        rule(mapping, number, "invoice.invoiceNumber", "TRIM");
        rule(mapping, due, "invoice.dueDate", "DATE_PARSE");
        rule(mapping, amount, "invoice.amount", "DECIMAL_PARSE");
        rule(mapping, currency, "invoice.currency", "TRIM");
    ...
    private SourceField field(SourceSchema s, String path, int pos, boolean key) {
    ...
    private void rule(MappingProfile p, SourceField s, String targetKey, String transform) {
        ...
        rules.saveAndFlush(new MappingRule(p.getId(), s.getId(), target.getId(), cfg, null, true));
    }
    ...
    private byte[] csv(String invoice, String amount) {
        return ("Invoice;Customer;Number;DueDate;Amount;Currency\n"
                        + invoice
                        + ";CUST-1;N-1;2026-09-01;"
                        + amount
                        + ";KZT\n")
    ...
    static class StorageConfig {
        @Bean
        @Primary
        ObjectStorage ingestionTestStorage() {
    ...
    static class MemoryStorage implements ObjectStorage {
        ...
        private String key(StorageLocation l) {
            return l.bucket() + "/" + l.objectKey();
        }

        public StoredObject upload(UploadObject c) {
            try {
                byte[] b = c.content().readAllBytes();
                values.put(key(c.location()), b);
                return new StoredObject(b.length, "test");
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
        }

        public InputStream download(StorageLocation l) {
            return new ByteArrayInputStream(Objects.requireNonNull(values.get(key(l))));
        }

        public ObjectMetadata stat(StorageLocation l) {
            byte[] b = Objects.requireNonNull(values.get(key(l)));
            return new ObjectMetadata(b.length, "application/octet-stream", "test");
        }

        public void delete(StorageLocation l) {
            values.remove(key(l));
        }

        public boolean exists(StorageLocation l) {
            return values.containsKey(key(l));
        }

        public URI generatePresignedGetUrl(StorageLocation l, Duration ttl) {
            return URI.create("https://test.invalid/get");
        }

        public URI generatePresignedPutUrl(StorageLocation l, String type, Duration ttl) {
            return URI.create("https://test.invalid/put");
        }
    }
}
