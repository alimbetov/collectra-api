...
class RabbitMqMessageDeliverySmokeIntegrationTest extends AbstractIntegrationTest {
    ...
    void outboxTraversesRealRabbitTopologyIntoDeliveryWorker() {
        Tenant tenant = tenants.saveAndFlush(new Tenant("rabbit-smoke-" + UUID.randomUUID(), "Rabbit Smoke"));
        locales.saveAndFlush(new TenantLocale(tenant.getId(), "ru", true, true, 0));
        var customer = customers.create(
                tenant.getId(), "RABBIT-" + UUID.randomUUID(), CustomerType.INDIVIDUAL,
                "Rabbit Customer", "Rabbit", "Customer", null, null, null, "ru", "Asia/Almaty",
                json.createObjectNode());
        customers.addEmail(tenant.getId(), customer.getId(), "rabbit-smoke@example.test", "WORK", true);

        DocumentTemplate template = templates.saveAndFlush(
                new DocumentTemplate(tenant.getId(), "RABBIT_" + UUID.randomUUID(), "Rabbit", "NOTIFICATION"));
        TemplateVersion version = new TemplateVersion(
                template.getId(), 1, "ru", TemplateChannel.EMAIL, "Rabbit smoke", "<p>Hello {{customer.name}}</p>", null);
        version.validated();
        version.publish();
        version = versions.saveAndFlush(version);

        var campaign = campaigns.create(
                tenant.getId(), "Rabbit smoke", version.getId(), CommunicationChannel.EMAIL.name(),
                null, CampaignSelection.customer(Set.of(customer.getId()), Set.of()), null);
        campaigns.activate(tenant.getId(), campaign.getId());
        var prepared = campaigns.prepare(tenant.getId(), campaign.getId());
        assertThat(materializer.materializeNextBatch(tenant.getId(), prepared.runId(), 100).queued()).isEqualTo(1);

        Message queued = messages.findAllByTenantIdAndCampaignRunId(
                tenant.getId(), prepared.runId(), PageRequest.of(0, 10)).getContent().get(0);
        ...
        await().atMost(Duration.ofSeconds(15)).pollInterval(Duration.ofMillis(100)).untilAsserted(() -> {
            ...
            assertThat(delivered.getProviderMessageId()).isEqualTo("simulated:email:" + messageId);
        });
    }
}
