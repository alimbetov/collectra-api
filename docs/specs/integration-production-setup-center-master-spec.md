...
- D3: service credentials are generated server-side and disclosed only in create/rotation mutation responses; safe list/detail DTOs expose only hints;
- I1 foundation: `IntegrationSource`, lifecycle, tenant-bound references, optimistic version commands, readiness API, `IngestionContext`, and human RBAC permissions;
...
Still intentionally open because they require further product/domain implementation: D4 read-only schema inspection; D6 permission composition; D8/D9 test-input contract choice; D10 source-oriented ingestion; D11/D12 new RBAC; D13/D14 readiness model; D15 concurrency semantics; D16 navigation; D17 stable problem codes; D18 polling/status contract; D19 readiness vs health UI; D20 route separation.
