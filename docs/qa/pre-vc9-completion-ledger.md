...
| WP-12 | remediation spec + WP review | cross-persona/system | full pre-VC9 chain | least privilege | tenant ownership at every boundary | composite Golden Journey + clean bootstrap | IMPLEMENTING | partial checkpoints implemented; T30-30 composite closure and external FH-008 rotation evidence remain | D13/D14; T30-30; FH-008 |
...
| T30-30 | top-30-smoke-contracts.md | cross-persona/system | integration -> finance -> collections -> campaign -> outbox/Rabbit -> delivery | least privilege by checkpoint | Alpha/Beta probes required | IntegrationPipelineExecutableSmokeTest + CollectionLifecycleApiIntegrationTest + RabbitMqMessageDeliverySmokeIntegrationTest | IMPLEMENTING | broker journey now includes service-auth, finance/allocation and collections checkpoints; source activation + authenticated idempotent ingestion are not yet one composite run | D13; WP-12 |
