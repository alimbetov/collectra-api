...
| T30-14 | WZ-14 | authenticate client, rotate/block, retry ingestion with old/new credentials/scopes | only currently valid scoped credential succeeds | service API | PASS |
| T30-15 | WZ-15 | customer detail after contract/invoice/payment/collection changes | accepted D05 Customer 360 links/summary refresh from authoritative APIs; no contradictory state | Browser+API | PASS |
| T30-16 | WZ-16 | two actors edit same versioned aggregate | stale command 409; first write preserved; UI reload/reapply required | API+Browser | PASS |
...
| T30-18 | WZ-18 | Alpha campaign selection contains Beta customer/segment | rejected/ignored per explicit contract without disclosure; no Beta recipient | API+DB | PASS |
| T30-19 | WZ-19 | publish v1, prepare/materialize, create/edit v2 | existing message/run remains bound to immutable intended content/version | API+DB | PASS |
| T30-20 | WZ-20 | malicious HTML/script/expression/control chars through template APIs | rejected/sanitized exactly per policy; preview/materialization safe | API+Frontend | PASS |
| T30-21 | WZ-21 | Alpha uses Beta file/asset in template/message and requests metadata/content | non-disclosure; no storage key/bucket leakage | API+DB | PASS |
...
| T30-26 | WZ-26 | stale PROCESSING + concurrent recovery/late event | one legal final transition; no backward terminal transition/double count | recovery+DB | PASS |
...
| T30-29 | WZ-29 | traverse every REAL/PARTIAL primary route with persona/deep link/filter/error | correct page/API/permission; no unexpected placeholder; URL state stable | Frontend | PASS |
...
The PASS rows above were executed together on branch SHA `a3dbd65bc20156eb044d6edea2485edde8f45401` in GitHub Actions run `36238480310` (backend verify and frontend both successful). T30-30 is exercised by `RabbitMqMessageDeliverySmokeIntegrationTest#outboxTraversesRealRabbitTopologyIntoDeliveryWorker`, which now composes service credential/token issuance, ACTIVE CUSTOMER/INVOICE/PAYMENT integration sources, idempotent ingestion, canonical finance/allocation, collections, published template, campaign materialization, outbox, real RabbitMQ, delivery worker and deterministic simulated provider. T30-26 is additionally exercised by `MessageRecoveryConcurrencyIntegrationTest#staleRecoveryRacingLateAcceptanceProducesOneLegalTerminalTransition`, which races stale recovery against a late provider acceptance behind a shared latch and asserts one SENT terminal transition, no retry scheduling and exactly one CampaignRun sent counter. No T30 row remains NOT_RUN; all rows still require the final exact-HEAD verification checkpoint before PRE-VC9_GATE_PASS. Additional PASS mappings were reconciled against existing focused suites: T30-07 `CollectionLifecycleApiIntegrationTest`; T30-10 `ReceivableFrontendApiIntegrationTest#moneyContractUsesStringsAndRejectsUnsafeDecimalForms` plus authoritative business-date projection; T30-14 service-client rotation/security plus `IngestionServiceAuthorizationIntegrationTest`; T30-15 `CustomerDetailPage.test.tsx` deep links and bounded customer-scoped reads; T30-18 `GenericCustomerCampaignAudienceIntegrationTest`; T30-19 `CampaignMessageMaterializationIntegrationTest#materializationSnapshotsTemplateVersionAcrossBatchesAndIsIdempotent`; T30-20 `HtmlTemplatePolicyUnitTest`; T30-21 `FileServiceApiIntegrationTest` and `FileControllerIntegrationTest`; T30-26 `MessageStateServiceTest` recovery/late-callback matrix; T30-28 `Slice10aP11MessageSecurityIntegrationTest`. These mappings still require the final-head CI checkpoint before PRE-VC9_GATE_PASS.
