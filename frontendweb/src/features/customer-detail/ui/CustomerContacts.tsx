      ...
      {!rows.length ? <EmptyState title={t('customerDetail.noEmails')} /> : (
        <div className="ui-data-table-scroll">
          <table className="ui-data-table" aria-label={t('customerDetail.emails')}>
            <thead><tr><th scope="col">{t('customers.columns.email')}</th><th scope="col">{t('customerDetail.contactType')}</th><th scope="col">{t('customers.columns.status')}</th><th scope="col">{t('customerDetail.flags')}</th></tr></thead>
...
export function CustomerPhones({ values }: { values: readonly CustomerPhoneDto[] }) {
  const { t } = useI18n();
  const rows = orderCustomerPhones(values);
  return (
    <section className="customer-detail-card customer-contact-section">
      <h2>{t('customerDetail.phones')}</h2>
      {!rows.length ? <EmptyState title={t('customerDetail.noPhones')} /> : (
        <div className="ui-data-table-scroll">
          <table className="ui-data-table" aria-label={t('customerDetail.phones')}>
            <thead><tr><th scope="col">{t('customers.columns.phone')}</th><th scope="col">{t('customerDetail.contactType')}</th><th scope="col">{t('customers.columns.status')}</th><th scope="col">{t('customerDetail.flags')}</th></tr></thead>
