...
export function SourceSchemasPage(){const qc=useQueryClient(),q=useQuery(integrationQueries.sourceSchemas()); const [code,setCode]=useState(''),[name,setName]=useState('');
 ...
 <div className="integration-list">{q.data?.map(x=><Link to={x.id} className="integration-list__row" key={x.id}><div><strong>{x.name}</strong><span>{x.code}</span></div><code>{x.id}</code></Link>)}</div></div>}
